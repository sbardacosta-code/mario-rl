...
REGRESSION_GUARD = {"minimum_seconds": 300, "window_episodes": 100, "minimum_completion_rate": 0.20}
...
class SessionCallback(BaseCallback):
    ...
    def _on_training_start(self):
        ...
        self.writer = csv.DictWriter(self.csv_file, fieldnames=["elapsed_seconds", "timesteps", "return", "length", "max_x", "completed", "game_score", "score_gain", "coins", "objective_reward_sum", "one_ups", "custom_one_up_bonus", "augmented_score_gain", "flag_height_bonus", "flag_touch_y", "flag_top_tier"])
    ...
    def progress(self):
        ...
        return {
            ...
            "recent_mean_augmented_score": float(np.mean([row["augmented_score_gain"] for row in recent])) if recent else None,
            "recent_mean_completed_augmented_score": float(np.mean([row["augmented_score_gain"] if row["completed"] else 0 for row in recent])) if recent else None,
            "recent_one_ups": sum(row["one_ups"] for row in recent),
            "recent_mean_flag_points_on_clears": float(np.mean([row["flag_height_bonus"] for row in recent if row["completed"]])) if any(row["completed"] for row in recent) else None,
    ...
    def _on_step(self):
        ...
        for done, info in zip(self.locals["dones"], self.locals["infos"]):
            if done:
                ...
                row = {
                    ...
                    "one_ups": int(metrics["one_ups"]),
                    "custom_one_up_bonus": int(metrics["custom_one_up_bonus"]),
                    "augmented_score_gain": int(metrics["augmented_score_gain"]),
                    "flag_height_bonus": int(metrics.get("flag_height_bonus") or 0),
                    "flag_touch_y": metrics.get("flag_touch_y"),
                    "flag_top_tier": int(bool(metrics.get("flag_top_tier"))),
                }
        ...
        if elapsed >= REGRESSION_GUARD["minimum_seconds"] and len(self.recent) >= REGRESSION_GUARD["window_episodes"] and sum(row["completed"] for row in self.recent) / len(self.recent) < REGRESSION_GUARD["minimum_completion_rate"]:
            self.stop_reason = "training_regression_requires_evaluation"
            return False
...
def main():
    ...
    parser.add_argument("--objective", choices=["score_events", "score_bridge"], default="score_bridge", help="settled game points and extra-life utility, optionally mixed with the original guidance")
    parser.add_argument("--score-weight", type=float, default=0.25, help="score reward share for score_bridge; score_events always uses 1.0")
    ...
    if not math.isfinite(args.score_weight) or not 0 < args.score_weight <= 1:
        parser.error("--score-weight must be in (0, 1]")
    if args.objective == "score_events":
        args.score_weight = 1.0
    if args.reward_scale != 1.0:
        parser.error("Overnight objectives require reward-scale 1.0")
    ...
    env = DummyVecEnv([
        lambda i=i: Monitor(ScaleReward(make_env(
            ...
            objective=args.objective, score_weight=args.score_weight,
    ...
    try:
        ...
        model.mario_score_weight = args.score_weight
        # SB3 saves prior episode statistics; never mix those old reward units
        # into the first hundred episodes under this new objective.
        ...
        config = {
            ...
            "event_reward_config": EVENT_REWARD_CONFIG,
            "regression_guard": REGRESSION_GUARD,
            "score_weight": args.score_weight,
            "reward": EVENT_REWARD_CONFIG["bridge_formula"] if args.objective == "score_bridge" else EVENT_REWARD_CONFIG["formula"],
            "reward_scale": args.reward_scale,
            "metric_units": {"episodes.csv:return": "native reward", "episodes.csv:score_gain": "actual game points earned including settled pole award, excluding later timer/fireworks bonuses", "episodes.csv:objective_reward_sum": "unscaled objective reward", "logs:rollout/ep_rew_mean": "scaled objective reward", "evaluation": "actual points and completion, plus native reward"},
