        ...
        for episode in evaluation.get("episodes", []):
            ...
            for part in ("beginning", "ending"):
                ...
                samples.append({
                    ...
                    "one_ups": episode.get("one_ups", 0),
                    "flag_height_bonus": episode.get("flag_height_bonus", 0),
...
def main():
    ...
    root = tk.Tk()
    root.title("Super Mario Bros. — overnight score training")
    ...
    def show_detail(score=None, coins=None, x=None, one_ups=0, flag_height_bonus=0):
        show = lambda value: "—" if value is None else f"{value:,}"
        detail.set(f"Game points {show(score)}  ·  Extra lives +{show(one_ups)}  ·  Pole +{show(flag_height_bonus)}")
    ...
    def select_clip(sample):
        ...
        try:
            ...
            show_detail(sample["score"], sample["coins"], sample["x"], sample.get("one_ups", 0), sample.get("flag_height_bonus", 0))
    ...
    tk.Label(root, text="Pause affects the viewer only. Recorded clips play twice.\n"
             "Extra lives earn a separate teaching bonus. Closing leaves training running.",
            ...
            if mode.get() == "Live training":
                ...
                if path and not state["paused"]:
                    ...
                    if stamp != state["live_stamp"]:
                        ...
                        show_detail(payload.get("score"), payload.get("coins"), payload.get("x"), payload.get("one_ups", 0), payload.get("flag_height_bonus", 0))
