...
def draw_chart(stages: list[dict], output: Path, session_id: str) -> None:
    ...
    axes[1, 1].set_title("Game points including the flagpole award" if score_data else "Prolonged runs without a new maximum x")
...
def make_report(repo: Path, manifest_path: Path, manifest: dict, stages: list[dict], document: Path, chart: Path, generated_at: str) -> str:
    ...
    if config.get("objective") in ("score_events", "score_bridge"):
        ...
        for stage in complete:
            ...
            content.append(f"| {cell(stage.get('label', stage['id']))} | {number(evaluation.get('mean_score_gain'), 1)} | {number(evaluation.get('mean_completed_score'), 1)} | {number(evaluation.get('mean_completed_augmented_score'), 1)} | {stage['clears']}/{stage['count']} |")
    ...
    for stage in stages:
        content += [f"### {cell(stage.get('label', stage['id']))}", "", f"Stage `{cell(stage['id'])}` · {number(stage['minutes'], 1)} additional minutes · {number(stage.get('training_timesteps'))} decisions represented by this checkpoint (a rollback can restore an earlier counter).", ""]
    ...
    content += ["To update this dashboard after generating new evaluations:", "", "```sh", f".venv/bin/python overnight_gallery.py --manifest {manifest_path.relative_to(repo).as_posix()} --repo-root .", "```", "", "The dashboard update remains local until it is committed and uploaded to GitHub. It does not run training or modify model weights.", ""]
    return "\n".join(content)
